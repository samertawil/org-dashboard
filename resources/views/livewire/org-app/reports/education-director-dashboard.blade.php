                        ...
                        <tbody class="divide-y divide-zinc-100 dark:divide-zinc-800">
                            @foreach ($supervisorReports as $report)
                                @php
                                    ...
                                    $domainClass =
                                        $domainColors[$report->domain_name] ??
                                ...
                                <tr class="hover:bg-orange-50/30 dark:hover:bg-orange-950/10 transition-colors">
                                    ...
                                    <td class="py-3 px-3 font-medium text-zinc-900 dark:text-zinc-100 min-w-[600px]">
                                        <span
                                            class="leading-snug break-words">{{ $report->activity_name ?? '—' }}</span>
                                    </td>

                                    <td class="py-3 px-3 whitespace-nowrap">
                                        @if ($report->domain_name)
                                            <span
                                                class="inline-block text-xs font-semibold px-2 py-0.5 rounded-full {{ $domainClass }}">
                                                {{ $report->domain_name }}
                                            </span>
                                        @else
                                    ...
                                    <td class="py-3 px-3 max-w-[200px]">
                                        @forelse ($report->group_names as $gname)
                                            <span
                                                class="block text-xs text-zinc-700 dark:text-zinc-300 leading-snug line-clamp-2">{{ $gname }}</span>
                                        @empty
                                            <span class="text-zinc-400">—</span>
                                        @endforelse
                                    </td>
                                    ...
                                    <td class="py-3 px-3 max-w-[90px]">
                                        @if ($firstBody && $firstBody->content)
                                            <div>
                                                <p
                                                    class="text-zinc-700 dark:text-zinc-300 text-xs leading-relaxed line-clamp-2">
                                                    {{ $firstBody->content }}
                                                </p>
                                                @if (mb_strlen($firstBody->content) > 50)
                                                    <button
                                                        @click="openContentModal($el.getAttribute('data-title'), $el.getAttribute('data-content'))"
                                    ...
                                    <td class="py-3 px-3 min-w-[245px] max-w-[245px]">
                                        @if ($firstBody && $firstBody->observation)
                                            <div>
                                                <p
                                                    class="text-zinc-600 dark:text-zinc-400 text-xs leading-relaxed line-clamp-2 italic">
                                                    {{ $firstBody->observation }}
                                                </p>
                                                @if (mb_strlen($firstBody->observation) > 50)
                                                    <button
                                                        @click="openContentModal($el.getAttribute('data-title'), $el.getAttribute('data-content'))"
                                                        data-title="ملاحظة: {{ $report->activity_name ?? 'النشاط' }}"
                                                        data-content="{{ $firstBody->observation }}"
                                                        class="text-blue-600 hover:text-blue-700 dark:text-blue-400 dark:hover:text-blue-300 text-xs font-semibold mt-1 cursor-pointer focus:outline-none block">
                                                        قراءة المزيد
                                                    </button>
                                                @endif
                                        ...
                                        @else
                                            <span class="text-zinc-350 dark:text-zinc-600 text-xs">لا يوجد</span>
                                        @endif
                                    </td>
                        ...
                        <div>
                            ...
                            <div class="grid grid-cols-2 sm:grid-cols-3 gap-3">
                                <template x-for="(att, idx) in allAttachments" :key="idx">
                                    <div
                                        class="group relative rounded-xl overflow-hidden border border-zinc-200 dark:border-zinc-700 bg-zinc-50 dark:bg-zinc-800 aspect-square">
                                        <template
                                            ...
                                            <a :href="'/storage/' + att.path" target="_blank" rel="noopener"
                                                ...
                                                <div
                                                    ...
                                                    <div class="bg-white/90 rounded-full p-2">
                                                        <svg class="size-4 text-zinc-700" fill="none"
                                        ...
                                        <template
                                            ...
                                            <a :href="'/storage/' + att.path" target="_blank" rel="noopener"
                                                ...
                                                <span class="text-xs text-center break-all line-clamp-2"
                                                    x-text="att.name"></span>
                                                <span
                                                    class="text-xs font-mono uppercase bg-zinc-200 dark:bg-zinc-700 px-1.5 py-0.5 rounded"
