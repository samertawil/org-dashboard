<div class="flex flex-col gap-6" x-data="{ mentionableUsers: @js($this->mentionableUsers) }">
    <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div>
            ...
            <flux:subheading>{{ __('Listing all activities entered into the system on a specific day.') }}
            </flux:subheading>
        </div>
        <div class="flex flex-wrap items-center gap-2 w-full sm:w-auto">
            <flux:input type="date" wire:model.live.debounce.1500ms="reportDate" class="flex-1 sm:flex-none" />
            <flux:button icon="printer" onclick="window.print()" class="flex-1 sm:flex-none">{{ __('Print Log') }}
            </flux:button>
            <flux:button variant="primary" icon="chat-bubble-left-right" wire:click="sendToWhatsApp"
                wire:loading.attr="disabled" class="bg-green-600 hover:bg-green-700 text-white w-full sm:w-auto">
                {{ __('Send to WhatsApp') }}
            </flux:button>
        </div>
    </div>
    ...
    @if ($exchangeRate)
        <div
            ...
            <div class="text-sm">
                ...
                <span class="text-indigo-600 dark:text-indigo-400">1 USD = {{ $exchangeRate->currency_value }}
                    NIS</span>
                <span class="text-xs text-indigo-400 ml-2">({{ __('As of') }}:
                    {{ $exchangeRate->exchange_date }})</span>
            </div>
        </div>
    @endif
            ...
            @forelse($activities as $activity)
                <flux:card class="overflow-hidden border-l-4 border-l-indigo-500">
                    <div class="flex flex-col md:flex-row justify-between gap-4 mb-6">
                        <div>
                            <div class="flex items-center gap-2 mb-1">
                                <flux:badge size="sm" color="zinc">{{ $activity->created_at->format('H:i') }}
                                </flux:badge>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                        ...
                        <div class="space-y-4">
                            <div>
                                <flux:label class="text-xs uppercase tracking-wider text-zinc-400">{{ __('Entered By') }}
                                </flux:label>
                            ...
                            <div>
                                <flux:label class="text-xs uppercase tracking-wider text-zinc-400">{{ __('Work Team') }}
                                </flux:label>
                        ...
                        <div class="space-y-4">
                            <div>
                                <flux:label class="text-xs uppercase tracking-wider text-zinc-400">
                                    {{ __('Parcels Distributed') }}</flux:label>
                                <div class="space-y-1 mt-1">
                                    @forelse($activity->parcels as $parcel)
                                        <div
                                            ...
                                            <span class="font-bold">{{ number_format($parcel->distributed_parcels_count) }}
                                                {{ $parcel->unit->status_name ?? '' }}</span>
                                        </div>
                                    @empty
                            ...
                            <div>
                                ...
                                <div class="space-y-1 mt-1">
                                    ...
                                    @empty
                                        <span
                                            class="text-xs text-zinc-400 italic">{{ __('No beneficiaries recorded') }}</span>
                                    @endforelse
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="space-y-4">
                            <div>
                                <flux:label class="text-xs uppercase tracking-wider text-zinc-400">{{ __('Attachments') }}
                                </flux:label>
                                <div class="flex flex-wrap gap-2 mt-1">
                    ...
                    <div class="mt-8 border-t border-zinc-100 dark:border-zinc-800 pt-6">
                        <div class="flex items-center justify-between mb-4">
                            <button wire:click="toggleComments({{ $activity->id }})"
                                ...
                                <span class="text-sm font-bold">{{ __('Comments') }}
                            ...
                            <div x-data="activityComments({{ $activity->id }}, mentionableUsers)" class="relative">
                                <div class="flex gap-2 items-end">
                                    ...
                                    <div class="flex-1 relative">
                                        <textarea x-ref="commentInput" x-on:input="onInput($event)" wire:model="newComments.{{ $activity->id }}"
                                            rows="1" placeholder="{{ __('Write a comment… type @ to mention') }}"
                                            ...
                                            x-on:keydown.enter.prevent="if(!$event.shiftKey && !showDropdown) { $wire.addComment({{ $activity->id }}); showDropdown = false; }"></textarea>
                                        ...
                                        <div x-show="showDropdown"
                                            ...
                                            <div x-show="showDropdown"
                                                ...
                                                <div
                                                    ...
                                                    <span
                                                        class="text-sm font-semibold text-zinc-800 dark:text-zinc-200">{{ __('Select a team member') }}</span>
    ...
    @canany(['student.index', 'student.show', 'manager.reports.all'])
        <div class="mt-12 space-y-8">
                    ...
                    <div class="space-y-3">
                        @forelse($attendanceStats as $stat)
                            <div
                                ...
                                <div class="flex flex-col">
                                    <span
                                        class="text-sm font-bold text-sky-700 dark:text-sky-300">{{ $stat->studentGroup->name ?? __('Group ID: ') . $stat->student_group_id }}</span>
                                    <span
                                        class="text-xs text-sky-600 dark:text-sky-400">{{ __('Educational Group') }}</span>
                                </div>
                                <div class="text-right">
                                    <div class="text-xl font-black text-sky-700 dark:text-sky-300">
                                        {{ $stat->total_entries }}</div>
            ...
            <div class="mt-8">
                <flux:heading size="lg" class="mb-4 flex items-center gap-2">
                    <flux:icon icon="calendar-days" class="size-5 text-teal-600" />
                    {{ __('Educational Activity Schedules') }}
                </flux:heading>

                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                    @forelse($educationalActivityDetails as $detail)
                        @php $schedule = $detail->educationalActivity; @endphp
                        @if ($schedule)
                            <div
                    ...
                    @empty
                        <div class="col-span-full text-center py-8 text-zinc-500 italic">
                            {{ __('No educational activity schedules recorded today.') }}
                        </div>
                    @endforelse
                </div>
            </div>
