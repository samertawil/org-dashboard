                    ...
                    <div class="p-4 space-y-3 hover:bg-zinc-50 dark:hover:bg-zinc-800/50 transition-colors">
                        <div class="flex justify-between items-start">
                            <div class="flex flex-col">
                                <span class="text-xs text-zinc-500">{{ $quote->submitted_at->format('Y-m-d H:i') }}</span>
                                <span class="text-sm font-bold text-zinc-900 dark:text-white">{{ $quote->vendor->name }}</span>
                            </div>
                            <div class="text-right">
                                <span class="text-sm font-bold text-indigo-600 dark:text-indigo-400 block">{{ number_format($quote->total_amount, 2) }}</span>
                                <span class="text-[10px] text-zinc-500">{{ $quote->currency->status_name ?? '-' }}</span>
                            </div>
                        </div>

                        <div class="flex flex-col gap-1">
                            <span class="text-xs text-zinc-500">{{ __('Purchase Requisition') }}:</span>
                            <a href="{{ route('purchase_request.show', $quote->purchase_requisition_id) }}" class="text-sm text-blue-600 hover:underline font-bold">
                                #{{ $quote->purchaseRequisition->request_number }}
                            </a>
                            @if($quote->purchaseRequisition->description)
                                <div x-data="{ expanded: false }" class="text-[11px] text-zinc-500 italic">
                                    <div :class="expanded ? '' : 'line-clamp-1'">
                                        {{ $quote->purchaseRequisition->description }}
                                    </div>
                                    @if(strlen($quote->purchaseRequisition->description) > 50)
                                        <button @click="expanded = !expanded" class="text-indigo-600 font-bold hover:underline mt-1">
                        ...
                        <div class="flex items-center justify-end gap-2 pt-2">
                            <span title="{{ __('View quotation details') }}">
                                <flux:button href="{{ route('quotation.show', $quote->id) }}" wire:navigate variant="ghost" size="xs" icon="eye" />
                            </span>
                            <span title="{{ __('View financial comparison matrix') }}">
                                <flux:button href="{{ route('purchase_request.comparison', $quote->purchase_requisition_id) }}" wire:navigate variant="ghost" size="xs" icon="presentation-chart-bar" color="indigo" />
                            </span>
                        </div>
                    </div>
                @empty
            ...
            <div class="hidden md:block">
                <table class="w-full divide-y divide-zinc-200 dark:divide-zinc-700">
                    ...
                    <tbody class="bg-white dark:bg-zinc-800 divide-y divide-zinc-200 dark:divide-zinc-700">
                            ...
                            <tr class="hover:bg-zinc-50 dark:hover:bg-zinc-700/50">
                                ...
                                <td class="px-6 py-4 text-right text-sm">
                                    <div class="flex items-center justify-end gap-2">
                                        <span title="{{ __('View quotation details') }}">
                                            <flux:button href="{{ route('quotation.show', $quote->id) }}" wire:navigate variant="ghost" size="sm" icon="eye" />
                                        </span>
                                        <span title="{{ __('View financial comparison matrix') }}">
                                            <flux:button href="{{ route('purchase_request.comparison', $quote->purchase_requisition_id) }}" wire:navigate variant="ghost" size="sm" icon="presentation-chart-bar" color="indigo" />
                                        </span>
                                    </div>
                                </td>
                            </tr>
                        @empty
                            <tr>
                                <td colspan="6" class="px-6 py-8 text-center text-sm text-zinc-500 italic">{{ __('No responses found.') }}</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
