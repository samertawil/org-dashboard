<div class="flex flex-col gap-6" x-on:modal-show.window="Flux.modal($event.detail.name).show()">
    <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        ...
        <div class="flex gap-2 w-full sm:w-auto">
            <span title="{{ __('Export all requisitions to an Excel file') }}" class="flex-1 sm:flex-none">
                <flux:button wire:click="export" icon="document-arrow-down" variant="outline" class="w-full">
                    {{ __('Export Excel') }}
                </flux:button>
            </span>
            @can('purchase_request.create')
                <span title="{{ __('Create a new purchase requisition') }}" class="flex-1 sm:flex-none">
                    <flux:button href="{{ route('purchase_request.create') }}" wire:navigate variant="primary" icon="plus" class="w-full">
                        {{ __('New Request') }}
                    </flux:button>
                </span>
            @endcan
    ...
    <div class="bg-white dark:bg-zinc-800 rounded-lg border border-zinc-200 dark:border-zinc-700 shadow-sm p-4 sm:p-6">
        ...
        @if ($search_number || $search_status_id || $search_vendor_id || $search_date || $search_year)
            <div class="flex items-center justify-end mb-4">
                <span title="{{ __('Clear all applied filters') }}">
                    <flux:button
        ...
        <div class="overflow-x-auto -mx-4 sm:-mx-6">
            <div class="px-4 sm:px-6 py-4 border-b border-zinc-200 dark:border-zinc-700 bg-zinc-50 dark:bg-zinc-900">
                <div class="flex items-center justify-between">
                    <p class="text-sm text-zinc-600 dark:text-zinc-400">
                        {{ __('Showing') }}
                        <span class="font-medium text-zinc-900 dark:text-white">{{ $purchaseRequisitions->firstItem() }}</span>
                        {{ __('to') }}
                        <span class="font-medium text-zinc-900 dark:text-white">{{ $purchaseRequisitions->lastItem() }}</span>
                        {{ __('of') }}
                        <span class="font-medium text-zinc-900 dark:text-white">{{ $purchaseRequisitions->total() }}</span>
                        {{ __('results') }}
                    </p>
                </div>
            </div>
            ...
            <div class="md:hidden divide-y divide-zinc-200 dark:divide-zinc-700">
                    ...
                    <div class="p-4 space-y-3 hover:bg-zinc-50 dark:hover:bg-zinc-800/50 transition-colors">
                        <div class="flex justify-between items-start">
                            <div class="flex flex-col">
                                <span class="text-sm font-bold text-zinc-900 dark:text-white">#{{ $pr->request_number }}</span>
                                <span class="text-xs text-zinc-500">{{ $pr->request_date ? $pr->request_date->format('Y-m-d') : '-' }}</span>
                            </div>
                            <flux:badge size="sm">{{ $pr->status->status_name ?? '-' }}</flux:badge>
                        </div>
                        
                        <div class="text-sm text-zinc-600 dark:text-zinc-300 line-clamp-2">
                            {{ $pr->description ?? '-' }}
                        </div>
                        ...
                        @if($pr->suggested_vendors->isNotEmpty())
                            <div class="flex flex-wrap gap-1">
                                @foreach($pr->suggested_vendors as $vendor)
                                    <flux:badge size="xs" variant="ghost" class="text-[10px]">{{ $vendor->name }}</flux:badge>
                                @endforeach
                            </div>
                        @endif

                        <div class="flex items-center justify-end gap-2 pt-2">
                            @php $attachmentCount = count($pr->attachments ?? []); @endphp
                            <span title="{{ __('View attachments and gallery') }}">
                                <flux:button href="{{ route('purchase_request.gallery', $pr->id) }}"
                                    wire:navigate icon="paper-clip" variant="ghost" size="xs"
                                    style="{{ $attachmentCount > 0 ? 'color: #3b82f6 !important;' : '' }}" class="relative">
                            ...
                            @can('purchase_request.create')
                                <span title="{{ __('Quick view details') }}">
                                    <flux:button wire:click="showDetails({{ $pr->id }})" variant="ghost" size="xs" icon="eye" />
                                </span>
                                <span title="{{ __('Edit requisition') }}">
                                    <flux:button href="{{ route('purchase_request.edit', $pr->id) }}" wire:navigate
                                        variant="ghost" size="xs" icon="pencil-square" />
                                </span>
                                <span title="{{ __('Delete requisition') }}">
                ...
                @empty
                    <div class="p-8 text-center text-sm text-zinc-500 italic">
                        {{ __('No purchase requisitions found.') }}
                    </div>
                @endforelse
            </div>
            ...
            <div class="hidden md:block">
                <table class="w-full divide-y divide-zinc-200 dark:divide-zinc-700">
                    ...
                    <tbody class="bg-white dark:bg-zinc-800 divide-y divide-zinc-200 dark:divide-zinc-700">
                                    ...
                                    <div class="flex items-center justify-end gap-2">
                                        ...
                                        @can('purchase_request.create')
                                            <span title="{{ __('View Details') }}">
                                                <flux:button wire:click="showDetails({{ $pr->id }})" variant="ghost" size="sm" icon="eye" />
                                            </span>
                                            <span title="{{ __('Edit') }}">
                                                <flux:button href="{{ route('purchase_request.edit', $pr->id) }}" wire:navigate
                        ...
                        @empty
                            <tr>
                                <td colspan="6" class="px-6 py-8 text-center text-sm text-zinc-500 italic">
