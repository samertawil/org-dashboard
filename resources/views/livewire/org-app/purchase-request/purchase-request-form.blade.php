<div class="flex flex-col gap-6">
    <div class="flex items-start justify-between">
        <div class="flex flex-col gap-1">
            ...
            <flux:subheading>{{ $subheading ?? __('Enter the details for the Purchase Requisition below.') }}</flux:subheading>
        </div>
        ...
        <form wire:submit="{{ isset($type) ? $type : 'save' }}" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            ...
            <flux:field>
             ...
             <flux:field>
                ...
                <flux:input type="date" wire:model="request_date" />
            ...
            <div class="md:col-span-2 lg:col-span-3">
                <flux:field>
                    ...
                    <flux:textarea wire:model="description" :placeholder="__('General description of the required supply...')" rows="3" />
            ...
            {{-- Justification --}}
             <div class="md:col-span-2 lg:col-span-3">
                <flux:field>
                    ...
                    <flux:textarea wire:model="justification" :placeholder="__('Why is this supply needed?')" rows="3" />
            ...
            <div class="col-span-1 md:col-span-2 lg:col-span-3" x-data="{
             ...
             <flux:field class="relative z-40">
                ...
                <flux:input type="date" wire:model="need_by_date" />
                <flux:error name="need_by_date" />
            </flux:field>
            ...
            <div class="md:col-span-2 lg:col-span-3 mt-6 relative z-0">
                <div class="flex items-center justify-between mb-4">
                    <flux:heading size="lg" class="text-blue-600 dark:text-blue-400">{{ __('Items') }}</flux:heading>
                ...
                <div class="space-y-4">
                    @foreach ($items as $index => $item)
                        <div wire:key="item-{{ $index }}" class="p-4 border rounded-lg border-zinc-200 dark:border-zinc-700 bg-zinc-50 dark:bg-zinc-800/50">
                            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                                {{-- Item Name --}}
                                <flux:field>
                                    <flux:label>{{ __('Item Name') }}</flux:label>
                                    <flux:input type="text" wire:model="items.{{ $index }}.item_name" placeholder="Item Name" />
                                ...
                                <flux:field>
                                    ...
                                    <flux:input type="number" wire:model="items.{{ $index }}.quantity" placeholder="1" />
                                </flux:field>
                                ...
                                <flux:field>
                                    ...
                                    <flux:input type="number" step="0.01" wire:model="items.{{ $index }}.unit_price" placeholder="0.00" />
                                </flux:field>
                                ...
                                <div class="md:col-span-2 lg:col-span-3">
                                     <flux:field>
                                        <flux:label>{{ __('Description') }}</flux:label>
                                        <flux:input type="text" wire:model="items.{{ $index }}.item_description" placeholder="Item Description" />
                                    </flux:field>
                                </div>
                                
                                {{-- Action --}}
                                <div class="flex items-end justify-end">
                                    <flux:button wire:click="removePurchaseRequisitionItem({{ $index }})" variant="ghost" icon="trash" class="text-red-500 hover:text-red-600" />
                                </div>
                            </div>
                        </div>
                    @endforeach
