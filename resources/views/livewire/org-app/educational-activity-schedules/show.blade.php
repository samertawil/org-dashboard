<div class="flex flex-col gap-6" x-data="{
    ...
    touchEndX: 0,
    images: [
        @if($schedule->educationalActivity && !empty($schedule->educationalActivity->attchments))
    ...
    @else
        <div class="flex items-center justify-between border-b border-zinc-100 dark:border-zinc-800 pb-3">
            ...
            <div class="flex gap-2">
                @can('educational-activity-schedules.create')
                    <flux:button href="{{ route('educational-activity-schedules.edit', $schedule) }}" wire:navigate
                        variant="primary" size="sm" icon="pencil-square">
                        {{ __('Edit') }}
                    </flux:button>
                @endcan
                <flux:button @click="$dispatch('modal-close', { name: 'schedule-show-modal' })" variant="ghost" size="sm" icon="x-mark">
                    {{ __('Close') }}
                </flux:button>
            </div>
        </div>
    ...
    <flux:modal name="schedule-lightbox" class="w-full max-w-3xl" variant="bare" @close="isCarouselOpen = false; zoomLevel = 1">
