<div class="flex flex-col gap-6">
                    ...
                    @forelse($this->subjects as $subject)
                        <tr class="hover:bg-zinc-50 dark:hover:bg-zinc-700/50 transition-colors">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm">
                                 ...
                                 <flux:badge color="{{ $subject->activation == 1 ? 'green' : 'zinc' }}" size="sm">{{ $statusEnum?->label() ?? '-' }}</flux:badge>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                <div class="flex items-center justify-end gap-2">
                                    <flux:modal.trigger name="attachments-modal">
                                        <div class="relative">
                                            <flux:button wire:click="selectSubject({{ $subject->id }})" variant="ghost" size="sm" icon="paper-clip" style="{{ $subject->subjects_attchments_count > 0 ? 'color: #3b82f6 !important;' : '' }}" />
                                    ...
                                    <flux:button href="{{ route('subject.edit', $subject->id) }}" wire:navigate variant="ghost" size="sm" icon="pencil-square" />
                                    <flux:button wire:click="delete({{ $subject->id }})" wire:confirm="{{ __('Are you sure?') }}" variant="ghost" size="sm" icon="trash" class="text-red-500 hover:text-red-600" />
                                </div>
                            </td>
                        </tr>
                    @empty
                        <tr>
                            <td colspan="6" class="px-6 py-8 text-center text-sm text-zinc-500">{{ __('No subjects found.') }}</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
    ...
    <flux:modal name="attachments-modal" class="md:w-[600px]">
        <div class="space-y-6">
            <flux:heading level="2" size="lg">{{ __('Attachments For') }}: {{ $selectedSubject?->name }}</flux:heading>
                            ...
                            <div wire:key="attachment-{{ $attachment['id'] ?? $index }}"
                                ...
                                <div class="flex items-center gap-2">
                                    ...
                                    <div class="flex flex-col">
                                        <span class="text-sm font-medium">{{ \Illuminate\Support\Str::limit(basename($attachment['attchment_path'] ?? ''), 25) }}</span>
