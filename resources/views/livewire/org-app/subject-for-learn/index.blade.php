<div class="flex flex-col gap-6">
    <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        ...
        @can('object.create')
        <span title="{{ __('Add a new learning subject') }}" class="w-full sm:w-auto">
            <flux:button href="{{ route('subject.create') }}" wire:navigate variant="primary" icon="plus" class="w-full">
                {{ __('Add Subject') }}
            </flux:button>
        </span>
        @endcan
            ...
            <div class="px-6 py-4 border-b border-zinc-200 dark:border-zinc-700 bg-zinc-50 dark:bg-zinc-900">
                <div class="flex items-center justify-between">
                    <p class="text-sm text-zinc-600 dark:text-zinc-400 py-2">
                        {{ __('Showing') }}
                        <span
                            class="font-medium text-zinc-900 dark:text-white">{{ $subjects->firstItem() }}</span>
                        {{ __('to') }}
                        <span
                            class="font-medium text-zinc-900 dark:text-white">{{ $subjects->lastItem() }}</span>
                        {{ __('of') }}
                        <span class="font-medium text-zinc-900 dark:text-white">{{ $subjects->total() }}</span>
                        {{ __('results') }}
                    </p>
                </div>
            </div>
                    ...
                    <div class="p-4 space-y-3 hover:bg-zinc-50 dark:hover:bg-zinc-800/50 transition-colors">
                        <div class="flex justify-between items-start">
                            <div class="flex flex-col">
                                <span class="text-sm font-bold text-zinc-900 dark:text-white">{{ $subject->name }}</span>
                                <span class="text-xs text-zinc-500">{{ $subject->type?->status_name ?? '-' }}</span>
                            </div>
                        ...
                        <div class="flex justify-between text-xs text-zinc-500">
                            <span>{{ __('Age') }}: {{ $subject->from_age ?? '-' }} - {{ $subject->to_age ?? '-' }}</span>
                            <span>{{ __('Attachments') }}: {{ $subject->subjects_attchments_count }}</span>
                        </div>
                ...
                <div class="space-y-4">
                    @can('object.create')
                        <div class="flex items-center justify-between">
                            ...
                            <span title="{{ __('Add another file to upload') }}">
                        ...
                        <div wire:key="new-attachment-{{ $index }}"
                            ...
                            @can('subject.create')
                                <div class="flex items-end gap-2">
                                    ...
                                    <span title="{{ __('Remove this file from upload list') }}">
                    ...
                    @if (!empty($newAttachments))
                        <div class="flex justify-end">
                            <span title="{{ __('Upload all newly selected files to the server') }}">
