<div x-data="offlineAttendance({
    ...
    <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div class="flex flex-col gap-1">
            ...
            <div x-show="online && unsyncedCount > 0" x-cloak
                class="text-blue-600 dark:text-blue-400 text-sm font-medium flex items-center gap-2">
    ...
    @forelse($groupedStudents as $statusId => $statusStudents)
        <div
            ...
            {{-- Status Group Header --}}
            <div
                class="px-4 py-3 bg-zinc-100 dark:bg-zinc-900 border-b border-zinc-200 dark:border-zinc-700 flex items-center justify-between">
                <div class="flex items-center gap-2">
                    ...
                    <span
                        class="inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-medium bg-zinc-200 text-zinc-600 dark:bg-zinc-700 dark:text-zinc-300">
            ...
            <div class="block md:hidden divide-y divide-zinc-200 dark:divide-zinc-700">
                @foreach ($statusStudents as $student)
                    <div
                        class="p-4 flex items-center gap-4 hover:bg-zinc-50 dark:hover:bg-zinc-800/50 transition-colors">
                        <div class="flex-shrink-0">
                            <div class="relative flex items-center w-fit">
                                <flux:checkbox x-model="attendance['{{ $student->id }}']"
                                    @change="updateStatus('{{ $student->id }}')" size="sm" />
                                <div x-show="attendanceStatus['{{ $student->id }}'] === 'absent' && !attendance['{{ $student->id }}']"
                        ...
                        <div class="flex-1 min-w-0">
                            <div class="flex justify-between items-start">
                                <span
                                    class="text-sm font-bold text-zinc-900 dark:text-white truncate">{{ $student->full_name }}</span>
                                @php
                                    ...
                                    <div class="relative flex items-center w-fit">
                                        ...
                                        <div x-show="attendanceStatus['{{ $student->id }}'] === 'absent' && !attendance['{{ $student->id }}']"
                                            ...
                                            <flux:icon name="x-mark"
                                                class="size-4 text-red-600 dark:text-red-500 font-bold" />
                                        </div>
                                    </div>
                                </td>
                                <td
    ...
    @empty
        <div
            class="bg-white dark:bg-zinc-800 rounded-lg border border-zinc-200 dark:border-zinc-700 shadow-sm overflow-hidden p-8 text-center text-sm text-zinc-500 italic">
            ...
            function registerOfflineAttendance() {
                ...
                Alpine.data('offlineAttendance', (config) => ({
                    ...
                    async syncOfflineData() {
                        ...
                        const pageRecords = records.filter(r => r.groupId === this.groupId && r.date === this
                            .date);
                        ...
                        if (pageRecords.length > 0) {
                            try {
                                ...
                                const deletePromises = pageRecords.map(r => window.OfflineAttendance
                                    .markAsSynced(r.id));
