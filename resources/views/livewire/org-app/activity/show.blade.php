<div class="flex flex-col gap-6 print:gap-4" x-data="{ activeTab: 'overview' }">
    ...
    <div class="flex flex-col  items-start justify-between print:hidden text-left">
        ...
        <div class="flex flex-wrap items-center gap-2 mt-3 w-full">
            ...
            <x-action-message on="summary-generated" class="text-green-600 dark:text-green-400">
                {{ __('Done!') }}
            </x-action-message>
            
            <x-action-message on="summary-error" class="text-red-600 dark:text-red-400">
                {{ __('Error!') }}
            </x-action-message>
                    ...
                    @if ($activity->summary)
                        <div class="p-4 bg-blue-50 dark:bg-blue-900/20 border border-blue-100 dark:border-blue-800 rounded-xl mb-4 w-full min-w-0 overflow-hidden">
                            <div class="flex items-center gap-2 mb-2">
                                <flux:icon icon="sparkles" class="text-blue-600 dark:text-blue-400 w-5 h-5" />
                                <span class="font-bold text-sm text-blue-800 dark:text-blue-300">{{ __('AI Intelligence Summary') }}</span>
                            </div>
                            <div class="text-sm text-blue-700 dark:text-blue-200 leading-relaxed whitespace-pre-wrap break-all">
                                {{ $activity->summary->summary_text }}
                            </div>
                            <div class="mt-2 text-[10px] text-blue-400 dark:text-blue-500">
                                {{ __('Generated by AI model:') }} {{ $activity->summary->model_used }} • {{ $activity->summary->updated_at->diffForHumans() }}
                            </div>
                        </div>
                    @endif
