<div class="flex flex-col gap-6 print:gap-4" x-data="{ activeTab: 'overview' }">
    ...
    <div class="border-b border-zinc-200 dark:border-zinc-700 print:hidden text-left">
        <nav class="-mb-px flex space-x-8" aria-label="Tabs">
            <button @click="activeTab = 'overview'" 
                :class="activeTab === 'overview' ? 'border-primary-500 text-primary-600 dark:text-primary-400' : 'border-transparent text-zinc-500 hover:text-zinc-700 hover:border-zinc-300 dark:text-zinc-400 dark:hover:text-zinc-300'"
                class="group inline-flex items-center py-4 px-1 border-b-2 font-medium text-sm transition-colors cursor-pointer">
                <flux:icon icon="information-circle" class="mr-2 h-5 w-5" x-bind:class="activeTab === 'overview' ? 'text-primary-500' : 'text-zinc-400 group-hover:text-zinc-500'"/>
                {{ __('Overview') }}
            </button>

            <button @click="activeTab = 'location'" 
                :class="activeTab === 'location' ? 'border-primary-500 text-primary-600 dark:text-primary-400' : 'border-transparent text-zinc-500 hover:text-zinc-700 hover:border-zinc-300 dark:text-zinc-400 dark:hover:text-zinc-300'"
                class="group inline-flex items-center py-4 px-1 border-b-2 font-medium text-sm transition-colors cursor-pointer">
                <flux:icon icon="map-pin" class="mr-2 h-5 w-5" x-bind:class="activeTab === 'location' ? 'text-primary-500' : 'text-zinc-400 group-hover:text-zinc-500'"/>
                {{ __('Location') }}
            </button>

            <button @click="activeTab = 'financials'" 
                :class="activeTab === 'financials' ? 'border-primary-500 text-primary-600 dark:text-primary-400' : 'border-transparent text-zinc-500 hover:text-zinc-700 hover:border-zinc-300 dark:text-zinc-400 dark:hover:text-zinc-300'"
                class="group inline-flex items-center py-4 px-1 border-b-2 font-medium text-sm transition-colors cursor-pointer">
                <flux:icon icon="currency-dollar" class="mr-2 h-5 w-5" x-bind:class="activeTab === 'financials' ? 'text-primary-500' : 'text-zinc-400 group-hover:text-zinc-500'"/>
                {{ __('Financials') }}
            </button>

            <button @click="activeTab = 'teams'" 
                :class="activeTab === 'teams' ? 'border-primary-500 text-primary-600 dark:text-primary-400' : 'border-transparent text-zinc-500 hover:text-zinc-700 hover:border-zinc-300 dark:text-zinc-400 dark:hover:text-zinc-300'"
                class="group inline-flex items-center py-4 px-1 border-b-2 font-medium text-sm transition-colors cursor-pointer">
                <flux:icon icon="users" class="mr-2 h-5 w-5" x-bind:class="activeTab === 'teams' ? 'text-primary-500' : 'text-zinc-400 group-hover:text-zinc-500'"/>
                {{ __('Teams') }}
            </button>
        </nav>
    </div>
            ...
            <div class="flex flex-col gap-6">
                ...
                <flux:card class="space-y-6">
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-y-6 gap-x-8">
                        <div class="text-left">
                             <span class="text-xs font-medium text-zinc-500 block mb-1">{{ __('Activity Name') }}</span>
                             <span class="text-base font-semibold text-zinc-900 dark:text-zinc-100">{{ $activity->name }}</span>
                        </div>
                        
                        <div  >
                            <span class="text-xs font-medium text-zinc-500 block mb-1 ">{{ __('Status') }}</span>
                         ...
                         <div class="text-left">
                            <span class="text-xs font-medium text-zinc-500 block mb-1">{{ __('Duration') }}</span>
                            <div class="flex items-center gap-2 text-sm text-zinc-900 dark:text-zinc-100">
                                <span>{{ $activity->start_date }}</span>
                                <flux:icon icon="arrow-right" variant="micro" class="text-zinc-400"/>
                                <span>{{ $activity->end_date ?? __('Ongoing') }}</span>
                            </div>
                        </div>

                        <div  >
                            <span class="text-xs font-medium text-zinc-500 block mb-1">{{ __('Total Cost') }}</span>
                            <span class="  text-zinc-900 dark:text-zinc-100">
                                ${{ number_format($activity->cost, 2) }} <span class="text-xs font-normal text-zinc-500">USD</span>
                            </span>
                        </div>

                        <div class="text-left mt-5">
                             <span class="text-xs font-medium text-zinc-500 block mb-1">{{ __('Rating') }}</span>
                             <div class="flex items-center gap-1">
                                <flux:icon icon="star" variant="solid" class="{{ $activity->rating_info['color'] }} w-4 h-4" />
                                <span class="text-xs font-bold text-zinc-900 dark:text-zinc-100">{{ $activity->rating_info['rating'] }}</span>
                             </div>
                        </div>
                    </div>
                ...
                {{-- Metadata Card --}}
                 <flux:card class="space-y-4 h-fit">
                    <flux:heading size="md" class="border-b border-zinc-100 dark:border-zinc-700 pb-3">{{ __('System Metadata') }}</flux:heading>
                    
                    <div class="flex flex-col gap-4">
                         <div class="flex justify-between items-center">
                            <span class="text-xs text-zinc-500">{{ __('Created By') }}</span>
                             <div class="flex items-center gap-2">
                                <flux:avatar src="" name="{{ $activity->creator->name ?? '?' }}" size="xs" />
                                <span class="text-xs font-medium text-zinc-900 dark:text-zinc-100">{{ $activity->creator->name ?? __('Unknown') }}</span>
                            </div>
                        </div>

                        <div class="flex justify-between items-center">
                            <span class="text-xs text-zinc-500">{{ __('System ID') }}</span>
                            <span class="text-xs font-mono text-zinc-700 dark:text-zinc-300 bg-zinc-100 dark:bg-zinc-800 px-2 py-0.5 rounded">#PJ-{{ str_pad($activity->id, 4, '0', STR_PAD_LEFT) }}</span>
                        </div>

                        <div class="flex justify-between items-center">
                             <span class="text-xs text-zinc-500">{{ __('Created At') }}</span>
                             <span class="text-xs text-zinc-700 dark:text-zinc-300">{{ $activity->created_at->format('M d, Y H:i') }}</span>
                        </div>

                        <div class="flex justify-between items-center">
                             <span class="text-xs text-zinc-500">{{ __('Last Updated') }}</span>
                             <span class="text-xs text-zinc-700 dark:text-zinc-300">{{ $activity->updated_at->format('M d, Y H:i') }}</span>
                        </div>
        ...
        <div x-show="activeTab === 'location'" class="print:block print:mt-4" style="display: none;">
             <flux:card class="max-w-3xl">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                     <div class="space-y-6">
                        <div>
                             <span class="text-xs font-medium text-zinc-500 block mb-1">{{ __('Region / Governorate') }}</span>
                             <div class="flex items-center gap-2">
                                <flux:icon icon="map" variant="outline" class="text-zinc-400 w-5 h-5"/>
                                <span class="text-base text-zinc-900 dark:text-zinc-100">{{ $activity->regions->region_name ?? '-' }}</span>
                             </div>
                        </div>
                         <div>
                             <span class="text-xs font-medium text-zinc-500 block mb-1">{{ __('City / Municipality') }}</span>
                             <div class="flex items-center gap-2">
                                <flux:icon icon="building-office-2" variant="outline" class="text-zinc-400 w-5 h-5"/>
                                <span class="text-base text-zinc-900 dark:text-zinc-100">{{ $activity->cities->city_name ?? '-' }}</span>
                             </div>
                        </div>
                     </div>
                     ...
                     <div class="space-y-6">
                         <div>
                             ...
                             <div class="flex items-center gap-2">
                                ...
                                <span class="text-base text-zinc-900 dark:text-zinc-100">{{ $activity->activityNeighbourhood->neighbourhood_name ?? '-' }}</span>
                             </div>
                        </div>
                         <div>
                             <span class="text-xs font-medium text-zinc-500 block mb-1">{{ __('Specific Location') }}</span>
                             <div class="flex items-center gap-2">
                                <flux:icon icon="map-pin" variant="outline" class="text-zinc-400 w-5 h-5"/>
                                <span class="text-base text-zinc-900 dark:text-zinc-100">{{ $activity->activityLocation->location_name ?? '-' }}</span>
                             </div>
                        </div>
                     </div>
                </div>
                ...
                @if ($activity->address_details)
                    <div class="mt-8 pt-6 border-t border-zinc-100 dark:border-zinc-700">
                        <span class="text-xs font-medium text-zinc-500 block mb-2">{{ __('Detailed Address / Directions') }}</span>
                         <div class="bg-zinc-50 dark:bg-zinc-900/50 p-4 rounded-lg border border-zinc-100 dark:border-zinc-700/50">
                            <p class="text-sm text-zinc-700 dark:text-zinc-300 leading-relaxed">{{ $activity->address_details }}</p>
                        </div>
                    </div>
                @endif
             </flux:card>
        ...
        <div x-show="activeTab === 'financials'" class="space-y-6 print:block print:mt-4" style="display: none;">
                        ...
                        <tbody class="divide-y divide-zinc-200 dark:divide-zinc-700">
                            ...
                            <tr>
                                <td class="px-4 py-3 font-medium text-zinc-900 dark:text-zinc-100">
                                    ...
                                    @if($parcel->notes)
                                        <div class="text-xs text-zinc-400 font-normal mt-0.5 truncate max-w-xs">{{ $parcel->notes }}</div>
                                    @endif
                                </td>
                                <td class="px-4 py-3">{{ $parcel->distributed_parcels_count }}</td>
                                <td class="px-4 py-3">${{ number_format($parcel->cost_for_each_parcel, 2) }}</td>
                                <td class="px-4 py-3 font-medium text-zinc-900 dark:text-zinc-100">
            ...
            <flux:card>
                <div class="flex items-center justify-between mb-4">
                     ...
                     <flux:badge size="sm" color="blue">{{ $activity->beneficiaries->count() }} {{ __('Groups') }}</flux:badge>
                </div>
                ...
                <div class="overflow-x-auto">
                                ...
                                <td class="px-4 py-3 font-medium text-zinc-900 dark:text-zinc-100">
                                    {{ $beneficiary->beneficiaryType->status_name ?? '-' }}
                                     @if($beneficiary->notes)
                                        <div class="text-xs text-zinc-400 font-normal mt-0.5 truncate max-w-xs">{{ $beneficiary->notes }}</div>
                                    @endif
                                </td>
                                <td class="px-4 py-3">{{ $beneficiary->beneficiaries_count }}</td>
                                <td class="px-4 py-3">${{ number_format($beneficiary->cost_for_each_beneficiary, 2) }}</td>
                                <td class="px-4 py-3 font-medium text-zinc-900 dark:text-zinc-100">
                 ...
                 @else
                    <div class="text-center py-6 text-zinc-400 italic">{{ __('No beneficiary data available.') }}</div>
                @endif
            </flux:card>
        </div>
             ...
             <flux:card>
                 ...
                 <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                    @foreach ($activity->workTeams as $team)
                        <div class="flex items-start gap-3 p-4 rounded-xl border border-zinc-200 dark:border-zinc-700 bg-white dark:bg-zinc-900 hover:shadow-sm transition-shadow">
                            <flux:avatar src="" name="{{ $team->employeeRel->full_name ?? '?' }}" />
                            <div class="flex flex-col flex-1 min-w-0">
                                <span class="text-sm font-semibold text-zinc-900 dark:text-zinc-100 truncate" title="{{ $team->employeeRel->full_name ?? '-' }}">
                                ...
                                @if($team->notes)
                                    <p class="text-xs text-zinc-500 italic truncate" title="{{ $team->notes }}">{{ $team->notes }}</p>
                                @endif
                            </div>
                        </div>
                    @endforeach
                 ...
                 @else
                    <div class="text-center py-12">
                        ...
                        <h3 class="text-sm font-medium text-zinc-900 dark:text-zinc-100">{{ __('No teams assigned') }}</h3>
                        <p class="text-xs text-zinc-500 mt-1">{{ __('There are no work teams currently assigned to this activity.') }}</p>
                    </div>
                 @endif
             </flux:card>
        </div>
    ...
    <style>
        @media print {
            ...
            /* Styling Adjustments */
             .bg-white, .bg-zinc-50, .bg-zinc-800, .bg-zinc-900 {
                background-color: white !important;
