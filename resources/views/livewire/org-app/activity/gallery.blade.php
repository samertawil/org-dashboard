    ...
    images: [
        @foreach($this->attachments as $attachment)
            @php
                ...
                $url = asset('storage/' . $attachment->attchment_path);
            @endphp
            @if($isImage)
                { url: '{{ $url }}', name: '{{ addslashes($attachment->notes ?? '') }}' },
            @endif
        @endforeach
    ],
    openCarousel(url) {
                            ...
                            <div class="h-24 w-full bg-black relative overflow-hidden flex items-center justify-center">
                                @if (in_array(strtolower(pathinfo($attachment->attchment_path, PATHINFO_EXTENSION)), [
                                    ...
                                    <div  style="width: 170px; height: 150px;">
                                        <a href="#" @click.prevent="openCarousel('{{ asset('storage/' . $attachment->attchment_path) }}')" title="{{ __('View') }}">  <img src="{{ asset('storage/' . $attachment->attchment_path) }}"
            ...
            <div class="space-y-4">
                ...
                >
                    ...
                    <div x-show="isUploading" class="mt-3">
                        <div class="w-full bg-zinc-250 dark:bg-zinc-700 h-2 rounded-full overflow-hidden">
