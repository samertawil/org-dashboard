<div class="flex flex-col gap-6">
    ...
    <div class="flex items-start justify-between">
        <div class="flex flex-col gap-1">
            <flux:heading level="1" size="xl">{{ __('Student Details') }}: {{ $student->full_name }}</flux:heading>
            <flux:subheading>{{ __('View comprehensive information and survey answers for this student.') }}</flux:subheading>
        </div>
        <div class="flex gap-2">
            <flux:button href="{{ route('student.index') }}" wire:navigate variant="ghost" icon="arrow-left" class="print:hidden">
                {{ __('Back to List') }}
            </flux:button>
            ...
            @can('student.create')
            <flux:button href="{{ route('student.edit', $student) }}" wire:navigate variant="primary" icon="pencil" class="print:hidden">
                {{ __('Edit Student') }}
            </flux:button>
            @endcan
        </div>
    ...
    <style>
        @media print {
            ...
            .bg-zinc-50, .bg-zinc-100, .bg-white {
                background-color: transparent !important;
            }
            .dark\:bg-zinc-800, .dark\:bg-zinc-900 {
                background-color: transparent !important;
            }
            .border, .border-zinc-200, .border-zinc-700 {
    ...
    <style>
        @media print {
            ...
            .bg-zinc-50, .bg-zinc-100, .bg-white, .dark\:bg-zinc-800, .dark\:bg-zinc-900 {
                background-color: transparent !important;
            }
            .border, .border-zinc-200, .border-zinc-700 {
            ...
            .grid {
                display: grid !important; /* Keep grid for layout control */
            }
            .md\:grid-cols-3 {
                display: block !important;
            }
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div class="md:col-span-1 flex flex-col gap-6">
            <div class="bg-white dark:bg-zinc-800 rounded-lg border border-zinc-200 dark:border-zinc-700 shadow-sm overflow-hidden p-6 print:p-2">
                <div class="flex items-center gap-4 mb-6 print:mb-2">
                    <div class="h-16 w-16 rounded-full bg-zinc-100 dark:bg-zinc-700 flex items-center justify-center text-zinc-500 dark:text-zinc-400 text-2xl font-bold print:hidden">
                        {{ strtoupper(substr($student->full_name, 0, 1)) }}
                    </div>
                    <div>
                        <h2 class="text-xl font-semibold text-zinc-900 dark:text-white print:text-lg">{{ $student->full_name }}</h2>
                        <p class="text-sm text-zinc-500 dark:text-zinc-400">{{ $student->identity_number }}</p>
                    </div>
                </div>
                
                <div class="space-y-4 print:space-y-1">
                    <div class="grid grid-cols-2 gap-y-4 gap-x-2 text-sm print:grid-cols-3 print:gap-y-1 print:gap-x-4">
                        <div class="info-pair">
                            <span class="text-zinc-500 min-w-fit">{{ __('Birth Date') }}:</span>
                            <span class="font-medium text-zinc-900 dark:text-zinc-100">{{ $student->birth_date ?? '-' }}</span>
                        </div>

                        <div class="info-pair">
                            <span class="text-zinc-500 min-w-fit">{{ __('Group') }}:</span>
                            <span class="font-medium text-zinc-900 dark:text-zinc-100">{{ $student->studentGroup?->name ?? '-' }}</span>
                        </div>
                        ...
                        <div class="info-pair">
                            ...
                            <span class="font-medium text-zinc-900 dark:text-zinc-100">{{ $student->group->status_name ?? '-' }}</span>
                        </div>
                    ...
                    <div class="pt-4 mt-4 border-t border-zinc-200 dark:border-zinc-700 print:pt-1 print:mt-1">
                        <div class="text-sm text-zinc-500 mb-1 print:text-xs font-semibold">{{ __('Notes') }}:</div>
        ...
        <div class="md:col-span-2">
            <div class="bg-white dark:bg-zinc-800 rounded-lg border border-zinc-200 dark:border-zinc-700 shadow-sm overflow-hidden">
                <div class="px-6 py-4 border-b border-zinc-200 dark:border-zinc-700 bg-zinc-50 dark:bg-zinc-900 print:px-2 print:py-1">
                    <h3 class="text-lg font-medium text-zinc-900 dark:text-white print:text-base">{{ __('Survey Answers') }}</h3>
                </div>
                        ...
                        <div class="overflow-x-auto">
                            <table class="w-full survey-table border-collapse">
                                <thead class="bg-zinc-50 dark:bg-zinc-900/50 print:bg-white">
                                    <tr>
                                        <th class="px-4 py-2 border text-sm font-semibold text-right">{{ __('Question') }}</th>
                                        <th class="px-4 py-2 border text-sm font-semibold text-right">{{ __('Answer') }}</th>
                                    </tr>
                                </thead>
                                <tbody>
