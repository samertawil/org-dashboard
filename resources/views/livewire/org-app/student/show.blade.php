<div class="flex flex-col gap-6">
    ...
    <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div class="flex flex-col gap-1">
            ...
            <flux:subheading class="hidden sm:block">
                {{ __('View comprehensive information and survey answers for this student.') }}
            </flux:subheading>
        </div>
        <div class="flex flex-wrap items-center gap-2 w-full sm:w-auto">
            ...
            <span title="{{ __('View grading scale results') }}">
                <flux:button wire:click="$set('showGradingScale', true)" variant="ghost" icon="chart-bar"
                    class="flex-1 sm:flex-none print:hidden">
                    {{ __('Scale') }}
                </flux:button>
            </span>
            <span title="{{ __('Print student report') }}">
                <flux:button onclick="printWithDynamicName()" variant="ghost" icon="printer"
                    class="flex-1 sm:flex-none print:hidden">
                    {{ __('Print') }}
                </flux:button>
            </span>
            @can('student.create')
                <span title="{{ __('Edit student details') }}">
                    <flux:button href="{{ route('student.edit', $student) }}" wire:navigate variant="primary"
                        icon="pencil" class="flex-1 sm:flex-none print:hidden">
                        {{ __('Edit') }}
                    </flux:button>
                </span>
            @endcan
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div class="md:col-span-1 flex flex-col gap-6">
            <div
                ...
                <div
                    class="flex flex-col sm:flex-row items-center sm:items-start gap-4 mb-6 print:mb-2 text-center sm:text-start">
                    <div
                ...
                <div class="space-y-3 print:space-y-1">
                    <div
                        ...
                        <div class="info-pair">
                            ...
                            <span
                                class="font-medium text-zinc-900 dark:text-zinc-100">{{ $this->studentData->student_exact_age_when_join ?? '-' }}
                    ...
                    @foreach ($lateSurveyStudentData as $data)
                        <div class="info-pair flex">
                            ...
                            <div class="font-medium text-zinc-900 dark:text-zinc-100">
                                {{ $data->section_name ?? $data->survey_for_section }}</div>
                        </div>
                    @endforeach
                                ...
                                <div class="overflow-x-auto rounded-lg border border-zinc-200 dark:border-zinc-700">
                                    <table class="min-w-full divide-y divide-zinc-200 dark:divide-zinc-700">
                                        <thead class="bg-zinc-50 dark:bg-zinc-900/50">
                                            <tr>
                                                <th colspan="5"
                                                    class="px-4 py-2 text-center text-xs font-bold text-zinc-600 bg-indigo-50/50 dark:bg-indigo-900/20">
                                                    {{ $pair->pre_name }} vs {{ $pair->post_name }}
                                                </th>
                                            </tr>
                                            <tr>
                                                <th scope="col"
                                                    class="px-3 py-2 text-right text-xs font-semibold text-zinc-500 uppercase">
                                                    {{ __('Domain') }}</th>
                                                <th scope="col"
                                                    class="px-3 py-2 text-center text-xs font-semibold text-zinc-500 uppercase">
                                                    {{ __('Pre') }}</th>
                                                <th scope="col"
                                                    class="px-3 py-2 text-center text-xs font-semibold text-zinc-500 uppercase">
                                                    {{ __('Post') }}</th>
                                        ...
                                        <tbody
                                            ...
                                            @foreach ($pair->domains as $domain)
                                                <tr class="hover:bg-zinc-50/50 dark:hover:bg-zinc-800/50">
                                                    <td
                                                        class="px-3 py-2 text-xs font-medium text-zinc-900 dark:text-zinc-100">
                                                        {{ $domain['name'] }}</td>
                                                    <td
                                                        class="px-3 py-2 text-xs text-center text-zinc-600 tracking-tighter">
                                                        {{ $domain['pre'] }}</td>
                                                    <td
                                                        class="px-3 py-2 text-xs text-center text-zinc-600 tracking-tighter">
                                                        {{ $domain['post'] }}</td>
                                                    <td class="px-3 py-2 text-xs text-center font-bold">
                                                        @if ($domain['diff'] !== null)
                                                            <span
                                                                class="{{ $domain['diff'] >= 0 ? 'text-emerald-600' : 'text-red-600' }}">
                                        ...
                                        @if ($pair->total)
                                            <tfoot class="bg-zinc-50/50 dark:bg-zinc-900/30">
                                                <tr class="font-bold">
                                                    <td class="px-3 py-2 text-xs text-indigo-700 dark:text-indigo-400">
                                                        {{ __('Total Progress') }}</td>
                                                    ...
                                                    <td class="px-3 py-2 text-xs text-center">
                                                        @if ($pair->total['diff'] !== null)
                                                            <span
                                                                class="{{ $pair->total['diff'] >= 0 ? 'text-emerald-700' : 'text-red-700' }}">
                        ...
                        @forelse ($student->surveyStudentanswers->unique('survey_no')->values() as $survey)
                            ...
                            <div class="md:hidden space-y-4 mb-8 print:hidden">
                                <div
                                    ...
                                    <h4
                                        class="text-base font-bold text-indigo-600 dark:text-indigo-400 flex items-center gap-2">
                                    ...
                                    @foreach ($student->surveyStudentanswers->where('survey_no', $survey->survey_no)->sortBy('question.question_order') as $answer)
                                        @php
                                            ...
                                            if (
                                                ...
                                                $valuesAr =
                                                    json_last_error() === JSON_ERROR_NONE && is_array($decodedAr)
                                                        ? $decodedAr
                                                ...
                                                foreach ($valuesAr as $val) {
                                                    ...
                                                    $options = is_string($answer->question->answer_options)
                                                        ? json_decode($answer->question->answer_options, true)
                                        ...
                                        <div
                                            class="bg-white dark:bg-zinc-800 p-4 rounded-xl border border-zinc-200 dark:border-zinc-700 shadow-sm space-y-3">
                                            <div class="flex items-start gap-3">
                                                <span
                                                    class="shrink-0 w-6 h-6 flex items-center justify-center rounded-full bg-zinc-100 dark:bg-zinc-700 text-xs font-bold text-zinc-500">
                                                    {{ $answer->question?->question_order }}
                                                </span>
                                                <div
                                                    class="text-sm font-bold text-zinc-900 dark:text-white leading-relaxed">
                                                    {{ $answer->question?->question_ar_text }}
                                                </div>
                                            </div>

                                            <div
                                                class="bg-emerald-50/50 dark:bg-emerald-900/10 p-3 rounded-lg border border-emerald-100/50 dark:border-emerald-800/30">
                                                <div
                                                    class="text-[10px] uppercase tracking-wider text-emerald-600 dark:text-emerald-400 font-bold mb-1">
                                                    {{ __('الإجابة') }}</div>
                                ...
                                <div
                                    class="p-4 bg-zinc-50 dark:bg-zinc-900/30 rounded-xl border border-dashed border-zinc-200 dark:border-zinc-700 text-[10px] text-zinc-500 space-y-1">
                                    @php
                                        ...
                                        $created = $student->surveyStudentanswers
                                            ->where('survey_no', $survey->survey_no)
                                            ->map(
                                                fn($ans) => $ans->created_at
                                                    ? \Carbon\Carbon::parse($ans->created_at)->format('Y-m-d')
                                                    : null,
                                            )
                                            ->filter()
                                            ->unique()
                                            ...
                                            <td colspan="3"
                                                ...
                                                @php
                                                    ...
                                                    $created = $student->surveyStudentanswers
                                                        ...
                                                        ->map(
                                                            fn($ans) => $ans->created_at
                                                                ? \Carbon\Carbon::parse($ans->created_at)->format(
                                                                    'Y-m-d',
                                                                )
                                                                : null,
                                                        )
                        ...
                        <div class="overflow-x-auto rounded-lg border border-zinc-200 dark:border-zinc-700">
                            <table class="w-full table-auto divide-y divide-zinc-200 dark:divide-zinc-700">
                                <thead class="bg-zinc-50 dark:bg-zinc-800/50">
                                    <tr>
                                        <th
                                            class="px-4 py-3 text-start text-xs font-semibold text-zinc-500 uppercase tracking-wider">
                                            {{ __('Domain') }}</th>
                                        <th
                                            class="px-4 py-3 text-center text-xs font-semibold text-zinc-500 uppercase tracking-wider">
                                            {{ __('Total Marks') }}</th>
                                ...
                                <tbody class="bg-white dark:bg-zinc-800 divide-y divide-zinc-200 dark:divide-zinc-700">
                                    @forelse ($this->studentGradingScale->where('survey_no', $survey->survey_no)  as $grade)
                                        <tr class="hover:bg-zinc-50 dark:hover:bg-zinc-700/50 transition-colors">
                                            ...
                                            <td
                                                class="px-4 py-3 text-sm text-center font-bold text-indigo-600 dark:text-indigo-400">
                                                {{ $grade->grade }}%
                                            </td>
                                            ...
                                            <td class="px-4 py-3 text-xs text-start max-w-xs">
                                                @if (isset($grade->need_processing) && $grade->need_processing)
                                                    <div
                                                        class="flex items-start gap-2 p-2 bg-orange-50 dark:bg-orange-900/20 border border-orange-100 dark:border-orange-800 rounded-lg text-orange-800 dark:text-orange-300">
                                                        <flux:icon name="exclamation-circle"
                                                            class="size-4 shrink-0 mt-0.5" />
                                                        <span
                                                            class="leading-tight">{{ $grade->need_processing }}</span>
                                                    </div>
                                                @else
                                                    <span
                                                        class="text-zinc-400 italic">{{ __('No specific intervention') }}</span>
                                                @endif
                                            </td>
                                        </tr>
                                    @empty
