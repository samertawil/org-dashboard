<div class="flex flex-col gap-6">
    <div class="flex items-start justify-between">
        <div class="flex flex-col gap-1">
            <flux:heading level="1" size="xl">{{ __('Activity Beneficiaries') }}</flux:heading>
            <flux:subheading>{{ __('Manage activity beneficiaries and their details.') }}</flux:subheading>
        </div>

        <div class="flex gap-2">
            @can('activity.beneficiaries.create')
            <flux:modal.trigger name="import-modal">
                <flux:button variant="ghost" icon="document-arrow-up">{{ __('Import Excel') }}</flux:button>
            </flux:modal.trigger>
            <flux:button href="{{ route('activity.beneficiaries.create') }}" wire:navigate variant="primary" icon="plus">
                {{ __('Add Beneficiary') }}
            </flux:button>
            @endcan
        </div>
            ...
            <div class="px-6 py-4 border-b border-zinc-200 dark:border-zinc-700 bg-zinc-50 dark:bg-zinc-900">
                <div class="flex items-center justify-between">
                    <p class="text-sm text-zinc-600 dark:text-zinc-400 py-2">
                        {{ __('Showing') }}
                        <span
                            class="font-medium text-zinc-900 dark:text-white">{{ $this->beneficiaries->firstItem() }}</span>
                        {{ __('to') }}
                        <span
                            class="font-medium text-zinc-900 dark:text-white">{{ $this->beneficiaries->lastItem() }}</span>
                        {{ __('of') }}
                        <span class="font-medium text-zinc-900 dark:text-white">{{ $this->beneficiaries->total() }}</span>
                        {{ __('results') }}
                    </p>
                </div>
            </div>
                ...
                <thead class="bg-zinc-50 dark:bg-zinc-900">
                    <tr>
                        ...
                        <th wire:click="sortBy('displacement_camps_id')"
                            ...
                            <div class="flex items-center gap-1">
                                {{ __('Camp') }}
                                @if ($sortField === 'displacement_camps_id')
                                    <flux:icon name="{{ $sortDirection === 'asc' ? 'chevron-up' : 'chevron-down' }}"
                                        class="size-3" />
                                @else
                                    <flux:icon name="chevron-up-down" class="size-3 text-zinc-300" />
                ...
                <tbody class="bg-white dark:bg-zinc-800 divide-y divide-zinc-200 dark:divide-zinc-700">
                    @forelse($this->beneficiaries as $beneficiary)
                        <tr class="hover:bg-zinc-50 dark:hover:bg-zinc-700/50 transition-colors duration-150">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-zinc-600 dark:text-zinc-300">
                                {{ $beneficiary->activity?->title ?? $beneficiary->activity?->name ?? '-' }}
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-zinc-600 dark:text-zinc-300">
                                {{ $beneficiary->displacementCamp?->name ?? '-' }}
                            </td>
                             <td class="px-6 py-4 whitespace-nowrap text-sm text-zinc-600 dark:text-zinc-300">
                                <div>{{ $beneficiary->receipt_date }}</div>
                                <div class="text-xs text-zinc-500">{{ $beneficiary->status?->status_name ?? '-' }}</div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                <div class="flex items-center justify-end gap-2">
                                    <flux:button href="{{ route('activity.beneficiaries.edit', $beneficiary) }}" wire:navigate
                                        variant="ghost" size="sm" icon="pencil-square" />
                                    <flux:button wire:click="delete({{ $beneficiary->id }})"
                                        wire:confirm="{{ __('Are you sure you want to delete this beneficiary?') }}"
    ...
    <flux:modal name="import-modal" class="md:w-96">
        <div class="p-6 space-y-6">
            <div class="flex flex-col gap-1">
                <flux:heading size="lg">{{ __('Import Activity Beneficiaries') }}</flux:heading>
                <flux:subheading>{{ __('Upload an Excel file to import beneficiaries.') }}</flux:subheading>
            </div>
