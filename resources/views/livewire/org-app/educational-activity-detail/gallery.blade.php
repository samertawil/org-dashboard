<div class="h-full flex flex-col gap-6" x-data="{
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div class="flex flex-col gap-1">
            @if(!$this->isModal)
                <div class="flex items-center gap-2 text-zinc-500">
                    <a href="{{ route('educational-activity-detail.index') }}" wire:navigate class="hover:text-zinc-900 dark:hover:text-white transition-colors">
                        {{ __('Educational Activity Report') }}
                    </a>
                    <flux:icon icon="chevron-right" size="xs" />
                    <span class="text-zinc-900 dark:text-white font-medium">{{ $detail?->educationalActivity?->activity_name }}</span>
                </div>
            @endif
        ...
        <div class="flex items-center gap-3">
            {{-- <flux:input wire:model.live.debounce.300ms="search" placeholder="{{ __('Search files...') }}" icon="magnifying-glass" class="w-full md:w-64" /> --}}
    ...
    <div class="flex-1 flex overflow-hidden gap-6">
        <div class="lg:w-64 flex-shrink-0 {{ $this->isModal ? 'hidden' : 'hidden lg:flex' }} flex-col gap-2">
            <flux:heading level="3" size="sm" class="mb-2 px-2">{{ __('Filters') }}</flux:heading>

            <button wire:click="$set('filterType', '')" class="w-full text-left px-3 py-2 rounded-lg text-sm font-medium transition-colors {{ $filterType === '' ? 'bg-zinc-100 dark:bg-zinc-800 text-zinc-900 dark:text-white' : 'text-zinc-500 hover:text-zinc-900 dark:hover:text-white hover:bg-zinc-50 dark:hover:bg-zinc-800/50' }}">
            ...
            @foreach ($this->allStatuses as $status)
                <button wire:click="$set('filterType', '{{ $status->id }}')" class="w-full text-left px-3 py-2 rounded-lg text-sm font-medium transition-colors {{ $filterType == $status->id ? 'bg-zinc-100 dark:bg-zinc-800 text-zinc-900 dark:text-white' : 'text-zinc-500 hover:text-zinc-900 dark:hover:text-white hover:bg-zinc-50 dark:hover:bg-zinc-800/50' }}">
                    <div class="flex items-center gap-2">
            ...
            @if ($attachments->isEmpty())
                <div class="flex flex-col items-center justify-center h-64 border-2 border-dashed border-zinc-200 dark:border-zinc-700 rounded-xl bg-zinc-50 dark:bg-zinc-900/50">
                    <div class="flex flex-col items-center text-center p-6">
                        ...
                        <flux:modal.trigger name="upload-modal">
                            <flux:button variant="ghost" class="mt-4" size="sm" icon="plus">{{ __('Upload File') }}</flux:button>
                        </flux:modal.trigger>    
                    </div>
                </div>
            @else
                <div class="grid grid-cols-3 {{ $this->isModal ? '' : 'md:grid-cols-5 lg:grid-cols-6 xl:grid-cols-8' }} gap-3">
                    @foreach ($attachments as $index => $attachment)
                        <div wire:key="attachment-{{ $index }}" class="group relative bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-xl overflow-hidden shadow-sm hover:shadow-md transition-all hover:border-zinc-300 dark:hover:border-zinc-600 flex flex-col">
                            <div class="h-24 w-full bg-zinc-100 dark:bg-zinc-900 relative overflow-hidden flex items-center justify-center">
                                ...
                                <a @if($isImage) href="#" @click.prevent="openCarousel('{{ Storage::url($attachment['path']) }}')" @else href="{{ Storage::url($attachment['path']) }}" target="_blank" @endif style="width: 170px; height: 150px;" loading="lazy" class="w-full h-full flex items-center justify-center group-hover:opacity-90 transition-opacity">
                                    @if ($isImage)
                                        <img src="{{ Storage::url($attachment['path']) }}" alt="{{ $attachment['name'] }}" class="w-full h-full object-contain">
                                    @else
                            ...
                            <div class="p-3 border-t border-zinc-100 dark:border-zinc-800 flex flex-col gap-0.5">
                                <h4 class="text-sm font-medium text-zinc-900 dark:text-gray-100 truncate" title="{{ $attachment['name'] }}">
                                    {{ $attachment['name'] }}
                                </h4>
                                <div class="flex items-center justify-between">
                                    ...
                                    <button wire:click="deleteAttachment({{ $index }})" wire:confirm="{{ __('Are you sure you want to delete this file?') }}" class="text-zinc-400 hover:text-red-600 dark:hover:text-red-500 transition-colors" title="{{ __('Delete') }}">
    ...
    <flux:modal name="upload-modal" class="w-full md:w-[500px]">
        <div class="space-y-6">
            ...
            <div class="space-y-4">
                ...
                <flux:input wire:model="uploadNotes" :label="__('Display Name (Optional)')" placeholder="e.g. Activity Photo" />
            </div>

            <div class="flex justify-end gap-2 mt-2">
                <flux:button variant="ghost" x-on:click="$dispatch('modal-close', { name: 'upload-modal' })">{{ __('Cancel') }}</flux:button>
