<div class="flex flex-col gap-6">
    <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        ...
        @if(!$this->isModal)
            <span title="{{ __('Return to list') }}" class="w-full sm:w-auto">
                <flux:button href="{{ route('educational-activity-detail.index') }}" wire:navigate variant="ghost" icon="list-bullet" class="w-full">
                    {{ __('List') }}
                </flux:button>
            </span>
        @endif
    ...
    <div class="bg-white dark:bg-zinc-800 rounded-lg border border-zinc-200 dark:border-zinc-700 shadow-sm p-6">
        <form wire:submit="{{ isset($type) ? $type : 'save' }}" class="grid grid-cols-1 {{ $this->isModal ? '' : 'md:grid-cols-2 lg:grid-cols-3' }} gap-6">
            
            <div class="{{ $this->isModal ? 'col-span-1' : 'md:col-span-2 lg:col-span-3' }} border-b border-zinc-100 dark:border-zinc-700 pb-2 mb-2">
            ...
            <flux:field>
                ...
                <flux:select wire:model="educational_activity_id">
                    ...
                    @foreach($this->activitySchedules as $schedule)
                        <option value="{{ $schedule->id }}">{{ $schedule->activity_name }} ({{ $schedule->period_start?->format('Y-m-d') }})</option>
                    @endforeach
                </flux:select>
            ...
            <div class="{{ $this->isModal ? 'col-span-1' : 'md:col-span-2 lg:col-span-3' }}">
                <flux:field>
                    ...
                    <flux:textarea wire:model="what_learned" :placeholder="__('Describe what was learned')" rows="3" />
            ...
            <div class="{{ $this->isModal ? 'col-span-1' : 'md:col-span-2 lg:col-span-3' }}">
                <flux:field>
                    ...
                    <flux:textarea wire:model="teacher_report_detail" :placeholder="__('Teacher report details')" rows="3" />
                    <flux:error name="teacher_report_detail" />
                </flux:field>
            </div>

            <div class="{{ $this->isModal ? 'col-span-1 flex-col sm:flex-row' : 'md:col-span-2 lg:col-span-3' }} flex items-center justify-end gap-2 mt-6 relative z-0">
                <span title="{{ isset($type) && $type === 'save' ? __('Create') : __('Update') }}">
                    <flux:button type="submit" variant="primary" icon="{{ isset($type) && $type === 'save' ? 'plus' : 'check' }}" wire:loading.attr="disabled">
                        ...
                        <span wire:loading wire:target="save">{{ isset($type) && $type === 'save' ? __('Saving...') : __('Updating...') }}</span>
                    </flux:button>
                </span>
            </div>
            
            <div class="{{ $this->isModal ? 'col-span-1' : 'md:col-span-2 lg:col-span-3' }} flex justify-end w-full text-end">
