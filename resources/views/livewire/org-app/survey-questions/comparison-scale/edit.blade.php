<div class="max-w-4xl mx-auto flex flex-col gap-6">
    <div class="flex flex-col gap-1">
        <flux:heading level="1" size="xl">{{ __('Edit Comparison Scale') }}</flux:heading>
        <flux:subheading>{{ __('Modify the smart evaluation rule.') }}</flux:subheading>
    </div>

    <flux:card>
        <form wire:submit="save" class="flex flex-col gap-6">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <flux:input wire:model="from_percentage" type="number" step="0.01" :label="__('From Difference %')" required />
                <flux:input wire:model="to_percentage" type="number" step="0.01" :label="__('To Difference %')" required />
            </div>

            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <flux:input wire:model="evaluation" :label="__('Evaluation Label')" required />
            ...
            <flux:textarea wire:model="description" :label="__('Description (Internal Note)')" />
