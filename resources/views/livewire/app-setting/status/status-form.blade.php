<div class="flex flex-col gap-6">
    <div class="flex items-start justify-between">
        ...
        <flux:button href="{{ route('status.index') }}" wire:navigate variant="primary" icon="list-bullet">
            {{ __('Status List') }}
        </flux:button>
    </div>
    ...
    <div class="bg-white dark:bg-zinc-800 rounded-lg border border-zinc-200 dark:border-zinc-700 shadow-sm p-6">
        ...
        <form wire:submit="{{ $type }}" class="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <flux:field>
                ...
                <flux:input type="text" wire:model="status_name" :placeholder="__('Enter status name')"
                    class="md:col-span-2" />
                <flux:error name="status_name" />
            </flux:field>
            {{-- Parent Status --}}
            <flux:select wire:model="p_id_sub" :label="__('Parent Status')">
                <option value="" class="text-gray-500 placeholder-gray-500">{{ __('None (Root Status)') }}
                </option>
                @foreach ($this->getParentStatuses as $parent)
        ...
        >
            ...
            {{-- System Name --}}
            <flux:select wire:model="used_in_system_id" :label="__('System Name')"
