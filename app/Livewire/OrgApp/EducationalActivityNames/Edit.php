...
{
    public EducationalActivityName $activityName;
    ...
    {
        return [
            'activity_name' => ['required', 'string', 'max:255', 'unique:educational_activity_names,activity_name,' . $this->activityName->id, new UniqueActivityCoreName($this->activityName->id)],
    ...
    public function mount(EducationalActivityName $educationalActivityName)
    {
        $this->activityName = $educationalActivityName;
        $this->activity_name = $educationalActivityName->activity_name;
        $this->activity_domain = $educationalActivityName->activity_domain;
        $this->available_in_active_groups = (bool) $educationalActivityName->available_in_active_groups;
        $this->description = $educationalActivityName->description ?: '';
        $this->teachers = $educationalActivityName->teachers ?: [];
        $this->activation = $educationalActivityName->activation;
    }
    ...
    public function save()
    {
        ...
        if ($this->activityName->isDirty()) {
            $this->activityName->save();
            session()->flash('message', __('Activity name updated successfully.'));
        } else {
