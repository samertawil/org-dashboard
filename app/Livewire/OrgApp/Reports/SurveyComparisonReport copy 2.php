...
class SurveyComparisonReport extends Component
{
    ...
    public function mount()
    {
        ...
        $this->updatedPreSurveyId(137);
    }

    public function updatedPreSurveyId($value)
    {
        $this->postSurveyId = $this->surveyMapping[$value] ?? null;
    }
    ...
    public function render()
    {
        ...
        $surveys = StatusRepo::statuses()->whereIn('id', array_keys($this->surveyMapping));
        $postSurveys = StatusRepo::statuses()->whereIn('id', array_values($this->surveyMapping));
        $selectedPostSurvey = $postSurveys->firstWhere('id', $this->postSurveyId);
        ...
        return view('livewire.org-app.reports.survey-comparison-report', [
            'surveys' => $surveys,
            'selectedPostSurvey' => $selectedPostSurvey,
            'groups' => $groups,
    ...
    protected function calculateReportData()
    {
        ...
        foreach ($students as $student) {
            ...
            foreach ($domains as $domain) {
                ...
                if ($max > 0) {
                    $hasDomainPost = ($post > 0);
                    $diffPercent = ($pre > 0 && $hasDomainPost) ? ($post / $max * 100) - ($pre / $max * 100) : null;
                    
                    $scale = $diffPercent !== null ? $this->findMatchInCollection($scales, $diffPercent, $domain->id) : null;

                    $studentData['domains'][$domain->id] = [
                        'name' => $domain->status_name,
                        'pre' => $pre,
                        'post' => $post,
                        'diff_percent' => $diffPercent !== null ? round($diffPercent, 1) : null,
                        'evaluation' => $scale->evaluation ?? ($pre > 0 && !$hasDomainPost ? __('Pending') : '-'),
            ...
            $hasPostData = ($totalPost > 0);

            if ($totalMax > 0 && $totalPre > 0 && $hasPostData) {
                ...
                $studentData['total'] = [
                    ...
                    'post' => $totalPost,
                    'diff_percent' => null,
                    'evaluation' => $totalPre > 0 && !$hasPostData ? __('Pending Post-Survey') : __('No Data'),
                    'color' => '#9ca3af',
                ];
            }
    ...
    protected function calculateGroupComparison()
    {
        ...
        foreach ($groups as $group) {
            ...
            foreach ($group->students as $student) {
                ...
                // Only compare if both surveys are answered
