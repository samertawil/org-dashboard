...
// Insert Section 5 before @assets
$needle = "\n    @assets\n        <script src=\"https://cdn.jsdelivr.net/npm/apexcharts\"></script>\n    @endassets\n</div>";
$replacement = $section5 . "\n    @assets\n        <script src=\"https://cdn.jsdelivr.net/npm/apexcharts\"></script>\n    @endassets\n</div>\n";

if (strpos($content, '@assets') !== false) {
    // Find the @assets and replace from there
    $pos = strrpos($content, '    @assets');
    if ($pos !== false) {
        $before = substr($content, 0, $pos);
        $newContent = $before . $section5 . "\n    @assets\n        <script src=\"https://cdn.jsdelivr.net/npm/apexcharts\"></script>\n    @endassets\n</div>\n";
        file_put_contents($filePath, $newContent);
        echo "SUCCESS: Section 5 added to blade file.\n";
        echo "New file size: " . strlen($newContent) . " bytes\n";
        echo "New line count: " . substr_count($newContent, "\n") . "\n";
    } else {
        echo "ERROR: Could not find @assets position.\n";
    }
} else {
    echo "ERROR: @assets not found in file.\n";
}
